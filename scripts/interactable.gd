...
# Scenes are expected to provide an `InteractArea` child with a
# `CollisionShape2D`, and optionally a `SolidBody` child with its own
# `CollisionShape2D` when the station should block player movement.
...
func _ready() -> void:
	area = get_node_or_null("InteractArea") as Area2D
	if area == null:
		push_error("Missing InteractArea child on '%s'. Add it in the scene." % name)
		return
	...
	if shape == null:
		push_error("Missing CollisionShape2D under InteractArea on '%s'. Add it in the scene." % name)
		return
...
func add_solid_rect(size: Vector2, offset: Vector2 = Vector2.ZERO) -> void:
	var body := get_node_or_null("SolidBody") as StaticBody2D
	if body == null:
		push_error("Missing SolidBody child on '%s'. Add it in the scene." % name)
		return
	body.collision_layer = 1
	body.collision_mask = 0
	var shape := body.get_node_or_null("CollisionShape2D") as CollisionShape2D
	if shape == null:
		push_error("Missing CollisionShape2D under SolidBody on '%s'. Add it in the scene." % name)
		return
